...
int main(int argc, char *argv[]) {
    ...
    char *debugenv = NULL;
    char efile[PATH_MAX];
    char xbuildenv[MAX_ENV_LEN];
    char fullpath[PATH_MAX + 128];
    ...
    if (strchr(argv[0], '/') == NULL) {
        ...
        if (argv[0][0] != '/') {
            ...
            if (getcwd(mypath, sizeof(mypath) - 1) == NULL) {
                perror("getcwd");
                return -1;
            }
    ...
    if ( realpath(fullpath, myrealpath) == NULL ) {
        perror("realpath");
        return -1;
    }
    ...
    dir = dirname(myrealpath);

    DEBUG("my dir: %s\n", dir);
    
    sprintf(xbuildenv, "XBUILD_DIR=%s", dir);
    putenv(xbuildenv);

    sprintf(efile, "%s/../libexec/xrun", dir);
